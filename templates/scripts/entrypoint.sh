...
#
# The retry loop makes the harness machine resilient to transient LLM provider
...
while true; do
    ...
    echo ""
    echo "=========================================="
    echo "  Harness Machine -- attempt $attempt"
    ...
    # 60-second loops that waste an entire night.
    ...
    if [ $duration -lt 60 ] && [ "$recipe_internal_fail" != "true" ]; then
        ...
        if [ $consecutive_structural_failures -ge $MAX_CONSECUTIVE_FAILURES ]; then
            ...
            POSTMORTEM_FILE="{{project_dir}}/.harness-machine-postmortem"
            {
                echo "=== Harness Machine Post-Mortem ==="
            ...
            # A human can resume by editing STATE.yaml (e.g. clearing blockers).
